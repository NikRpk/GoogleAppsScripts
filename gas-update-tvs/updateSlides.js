function getSlideData() {
  ...
  while (files.hasNext()) {
    ...
    if (!slideData[tor]) {
      ...
      logger("Double Tor Slide", `Tor "${tor}" already exists! Please only have one.`);
    };
  };
  ...
  for (var slide in slideData) {
    if (slideData.hasOwnProperty(slide)) { // Check if the property belongs to the object
      ...
      if (routes[tor] && routes[tor].length > 0) {
        ...
        Logger.log(`Slide ${slideData[slide].tor} updated!`)
        ...
        Logger.log(`No tours planned for Tor ${tor}`)
      };
...
function updateSlide_(presentation, dict) {
  ...
  for (var i = 0; i < shapes.length; i ++) {
    ...
    if (altText in dict) {
      ...
      logger(`Did not find "${altText}" in the dictionary.`);
    };
  };
...
function addQrCode_(slides, laneName) {
  ...
  if (image) {
    ...
    logger("Missing QR code", `No QR code for the "${laneName}" lane could be found in the folder`);
  };
};
